...
std::ostream& operator<<(std::ostream &os, const RedBlackTree<Key,Val>& bst) {
    return os << "BST (r=right child, l=left child)\n" << bst.toString() << "\n";
}
...
int main() {
    ...
    while(iter < max_iter) {
        ...
        std::cout << num_initial_puts << " initial puts with random keys took " << exec_duration.count() * 1000 << " ms\n";
        ...
        std::cout << exec_duration.count()*1000 << " ms ("<<exec_duration.count()/1000*1e6<<" us/put)\n---\n";
